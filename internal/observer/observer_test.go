...
func TestSaveToBuffer_Success(t *testing.T) {
	// Register zbxpkg.History for gob
	gob.Register(zbxpkg.History{})
...
func TestSaveToBuffer_DBClosed(t *testing.T) {
	...
	exports := []zbxpkg.History{{ItemID: 1, Name: "foo"}}
	ttl := 1 * time.Hour
	err = saveToBuffer(db, exports, ttl)
	if err == nil {
...
func TestFetchfromBuffer_Success(t *testing.T) {
	...
	// Insert some zbxpkg.History values
	exports := []zbxpkg.History{
		{ItemID: 1, Name: "foo"},
		{ItemID: 2, Name: "bar"},
	}
	...
	// Fetch from buffer
	result, err := fetchfromBuffer[zbxpkg.History](db, 10)
	if err != nil {
